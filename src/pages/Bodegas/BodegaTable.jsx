...
export const BodegaTable = () => {
    ...
    const deleteBodega = async(id)=>{
        try {
            ...
            if(confirmDelete){
                const { data  } = await axios.delete(`http://localhost:3000/cellars/delete/${id}`,{headers: headers})
                getBodegas()
                alert(data.message)
                console.log(data)
            }
    ...
    /* const getAvailable = async () => {
        try {
            const { data } = await axios.post('http://localhost:3000/cellars/getAva', {headers: headers})
            setCellars(data.cellars)
            console.log(data)
        } catch (err) {
            console.log(err)
    ...
    const getNotAvailable = async () => {
        try {
            const { data } = await axios.post('http://localhost:3000/cellars/getInAva', {headers: headers})
            setCellars(data.cellars)
            console.log(data)
        } catch (err) {
            console.log(err)
    ...
    useEffect(() => { getBodegas();/*  getAvailable(); getNotAvailable() */ }, []);
    return (
        <>
        <br /><br /><br /><br /><br /><br /><br /><br /><br /><br /><br /><br /><br /><br /><br />
            <div className=' position-absolute top-50 start-50 translate-middle'>
                ...
                <div className="btn-group d-flex flex-wrap flex-column" role="group">
                    <button onClick={()=> getBodegas()} type="button" className="btn btn-primary mb-2 btn-block">All</button>
                   {/*  <button onClick={()=> getAvailable()} type="button" className="btn btn-primary mb-2 btn-block">Available</button>
                    <button onClick={()=> getNotAvailable()} type="button" className="btn btn-primary mb-2 btn-block">Not Available</button> */}
                </div>
