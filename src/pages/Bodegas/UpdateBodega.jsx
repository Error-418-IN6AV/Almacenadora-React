...
export const UpdateBodega = () => {
    ...
    const updateBodega = async()=>{
        try {
            ...
            const { data } = await axios.put(`http://localhost:3000/cellars/updateCellar/${id}`, updateBodega,{headers: headers} )
                alert(data.message)
                console.log(data)
        } catch (err) {  
           console.error(err)
    ...
    <>
        <form className='m-5 text-center'>
            ...
            <div>
             ...
             <Link to='/dashboard/bodegas'> <button className='btn btn-success' onClick={()=> updateBodega()}>Update</button> </Link>
