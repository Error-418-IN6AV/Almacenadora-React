...
export const Table = ()=>{
    const [fakeUser, setFakeUser] = useState([
        {name: 'jose', surname: 'cifuentes', username: 'admin', role: 'ADMIN'},
        {name: 'gabriel', surname: 'pinzon', username: 'worker', role: 'worker'}
    ])
    ...
    const deleteUser = async(id)=>{
        try{
            ...
            if(confirmDelete){
                ...
                getProducts()
                alert(`${data.message}: ${data.deletedUser.name}`)
            }
        }catch(err){
            console.error(err)
        }
    ...
    return (
        <>  
            <Link to='add'> 
                <button className="btn btn-success mb-5">ADD USER</button>
            </Link>
            <table>
                <thead>
                    <tr>
                ...
                <tbody>
                    {
                        users.map(({_id, name, surname, username, role}, index)=>{
                            return (
                                <tr key={index}>
                                    <User
                                        name={name}
